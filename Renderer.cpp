...
void Renderer::Draw()
{
	...
	if (mIsImageFirstTime[mImageIndex]) {
		VkImageMemoryBarrier mem{};
		mem.sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER;
		mem.pNext = nullptr;
		mem.srcAccessMask = VK_ACCESS_MEMORY_WRITE_BIT;
		mem.dstAccessMask = VK_ACCESS_MEMORY_WRITE_BIT;
		mem.oldLayout = VK_IMAGE_LAYOUT_UNDEFINED;
		mem.newLayout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;
		mem.srcQueueFamilyIndex = mGraphicsQueueIndex;
		mem.dstQueueFamilyIndex = mGraphicsQueueIndex;
		mem.image = mImages[mImageIndex];
		mem.subresourceRange.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;
		mem.subresourceRange.baseArrayLayer = 0;
		mem.subresourceRange.layerCount = 1;
		mem.subresourceRange.baseMipLevel = 0;
		mem.subresourceRange.levelCount = 1;
	}
